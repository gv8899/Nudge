...
- **既有舊帳號處理**：migration 內把所有現存 user 的 `onboarded_at` 一次性設為**固定過去時間戳（epoch `1970-01-01`）**，而非 `now()`。既標記「已 onboard」（seed 的 `WHERE onboarded_at IS NULL` 永不對他們觸發），又不會被前端「近 7 天」的近期窗判為剛 onboard（用 `now()` 會害每個既有用戶一連 7 天跳 welcome）。新帳號由 seed writer 寫真正的 `now()`。
...
| 既有舊帳號 | migration 一次性設 `onboarded_at = epoch(1970)` → 不 seed 舊帳號、也不誤跳 welcome |
