...
- 處理原則：iOS IAP 價格**設成貼近 web 的 Apple 階**；多出來的 15% 由我方吸收，或 iOS 略高。**美國/EU 可在 App 內直接告知「網頁訂閱更省」**（anti-steering 已放寬）。
...
- **Phase 1**：後端 entitlement schema + Web 訂閱（Paddle）+ 試用邏輯 + 四端 paywall/讀取。
- **Phase 2**：iOS StoreKit IAP（+ RevenueCat）+ entitlement 合流；Mac 導去 web 結帳。
